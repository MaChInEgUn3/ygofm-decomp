...
1239 of 1794 functions decompiled and byte-matching.
...
Do not restate any of these numbers anywhere else in the documentation. Both past miscounts in this repo -- the 1,198-versus-1,121 scope figure and the 167 -> 136 -> 66 `lui $at` pool -- were hand-derived numbers that outlived the batch that made them true.
...
Every ported function is in (1239 on the same day; 108 PARKED.txt entries
carry today's MATCH line by `grep -c`). The three that were parked as a
compiler disagreement between SN's cc1psx and his mips-sony-psx-gcc were a
front-end marking difference on `*(T *)&p->member` (struct-marked here,
not there; `MEM_IN_STRUCT_P` is what sched.c's `true_dependence` reads),
closed by the byte-address cast at five sites; MERGE_KRYSTALGAMER.md has
the measurements and the installed files name their divergent lines.
