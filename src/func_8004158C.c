/* MATCH, 2026-09-13. Escrita do zero no mesmo dia.
 ...
 *
 * 15. `ta` declarado u16 e nao s32: fecha as tres ultimas diferencas, a
 *     ordem de dois `andi` independentes no braco h+0x1C == 0 (3 -> MATCH).
 *     Veio do permuter (output-210-1) como `short ta`; s16 da o mesmo
 *     MATCH, e u16 e o tipo da leitura (`*(u16 *)(h + 0xC)`). Antes disso,
 *     sete grafias a mao do `ta &= 0x300` (antes do qC, qC em dois
 *     statements, embutido em q10, forma nao composta, do/while, nomes
 *     para h18 e r0, `t0 +=` depois do qC) e as flags -fno-schedule-insns
 *     e -fno-schedule-insns2 nao moviam a ordem: era o tipo do nome, nao
 *     a posicao do statement.
 *
