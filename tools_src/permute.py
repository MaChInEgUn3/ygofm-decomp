...
def write_compile_script(dest: Path, func: str):
    ...
    with io.open(dest, "w", encoding="utf-8", newline="\n") as f:
        f.write(
            ...
            + (f'export PERMUTER_CC_FLAGS="{os.environ["PERMUTER_CC_FLAGS"]}"\n'
               if os.environ.get("PERMUTER_CC_FLAGS") else "")
            +
            f'export PERMUTER_PRELUDE="{(dest.parent / "prelude.h").as_posix()}"\n'
...
def main():
    ...
    # A parked candidate measured at flags build.py has no row for says so in
    # its header ("FLAGS: -quiet -O2 -G8 -fno-strength-reduce"). Without this
    # the permuter compiled func_800323F8 at the default row: base score 2520
    # against 385 at the header's flags (2026-09-13).
    if (args.func not in B.PER_FUNC_FLAGS
            and not os.environ.get("PERMUTER_CC_FLAGS")):
        import re as _re
        m = _re.search(r"FLAGS:\s*(-quiet(?:\s+-[\w=-]+)+)", src.read_text())
        if m:
            flags = m.group(1).split()
            os.environ["PERMUTER_CC_FLAGS"] = " ".join(flags)
            print(f"flags from {src.name} header: {' '.join(flags)}")
    ...
    # Stripping __attribute__ also deleted every section(".data") arm, so
    # base.c compiled to a different program: func_8003DC1C 519/532 and
    # func_800262D4 471/474 against their parked 532/532 and 474/474, and two
    # permuter runs (2026-09-13) scored that program. Keep the attributed
    # declarations; permuter_compile.py re-inserts them before the function.
    full = subprocess.run(["gcc", "-E", "-P", "-I", str(ROOT / "include"),
                           str(src)], capture_output=True, text=True,
                          check=True).stdout
    attributed = [l for l in full.splitlines()
                  if "__attribute__" in l and l.rstrip().endswith(";")
                  and not l.lstrip().startswith("#")]
    (work / "prelude.h").write_text("\n".join(attributed) + "\n")
