...
set -e
# Wine has no display here and would otherwise log a window-driver
# error for every one of the thousands of invocations a search makes.
...
{" ".join(B.PSYQ_RUNNER)} "{B.CC1PSX.as_posix()}" {' '.join(flags)} "$INPUT" -o "$ASM"
...
def main():
    ...
    base = work / "base.c"
    B.run([*B.PSYQ_RUNNER, B.CPPPSX, *B.CPP_FLAGS,
