...
----------------------------------------------
Reconstruct func_80057544's 53-difference state by renaming t0..t4 back to a
single `t`, and this tool must report `case 0  t -> t_0 ... 29 differing` or
better. A run that finds nothing there is a broken tool, not a negative.
"""
import os
import re
import subprocess
...
def arms(text):
    """[(label, start, end)] for each top-level `case`/`default` arm.
...
def splittable(block, name):
    ...
    lab = re.search(r"\n *[A-Za-z_][A-Za-z_0-9]*:", block)
    if lab and occ[-1].start() > lab.start():
...
def main():
    ...
    if "--list" in sys.argv:
        ...
        for lab, lo, hi in blocks:
            names = [n for n, _ in decls if splittable(text[lo:hi], n)]
